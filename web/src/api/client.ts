...
export async function login(username: string, password: string): Promise<string> {
  const response = await withRetry(() => fetch(`${API_BASE}/api/auth/login`, {
    method: 'POST',
...
export async function listFiles(token: string, q = ''): Promise<FileItem[]> {
  const response = await withRetry(() => fetch(`${API_BASE}/api/files${q ? `?q=${encodeURIComponent(q)}` : ''}`, {
...
export async function uploadFile(token: string, file: File): Promise<FileItem> {
  ...
  const response = await withRetry(() => fetch(`${API_BASE}/api/files/upload`, {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + token },
    body: formData,
  }))
...
export async function createShareLink(token: string, fileKey: string): Promise<ShareResponse> {
  const response = await withRetry(() => fetch(`${API_BASE}/api/files/${fileKey}/share`, {
    method: 'POST',
    headers: {
...
export async function downloadFile(token: string, fileKey: string): Promise<void> {
  const response = await withRetry(() => fetch(`${API_BASE}/api/files/${fileKey}/download`, {
    headers: { Authorization: 'Bearer ' + token },
  }))
